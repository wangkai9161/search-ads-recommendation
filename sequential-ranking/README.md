...
#### 泄漏隔离的多随机种子评估（2026-09-20）

在 RTX 5080、MovieLens-1M、评分 `>=3` 的正反馈上，每个用户按时间留出最后两个目标：
倒数第二个只用于选择 epoch，最后一个只做一次测试。GRU4Rec/SASRec 使用历史前缀构造
最多 200,000 个训练样本；完整物品库评估时过滤已知正反馈。表中为 3 个随机种子的
均值和总体标准差：

| 模型 | Recall@20 | NDCG@20 |
| --- | ---: | ---: |
| Popularity | 0.067241 +/- 0.000000 | 0.024795 +/- 0.000000 |
| Two-Tower | 0.064591 +/- 0.002042 | 0.023828 +/- 0.000993 |
| GRU4Rec | **0.090979 +/- 0.002772** | **0.033728 +/- 0.001643** |
| SASRec | 0.064646 +/- 0.006707 | 0.024493 +/- 0.002478 |

修正测试集选参和序列样本不足后，GRU4Rec 稳定最佳；SASRec 在当前 10 epoch、
200k 前缀预算下波动较大。旧的 “SASRec 最佳” 结论不再采用。完整结果见
