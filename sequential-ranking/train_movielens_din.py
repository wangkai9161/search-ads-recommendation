...
def build_din_samples(df, max_history_len=20):
    """
    Build DIN samples.
    ...
    For each user, only previous interactions are used as history.
    """
...
def split_by_time(samples, train_ratio=0.8, valid_ratio=0.1):
    ...
    train_df = samples.iloc[:n_train].reset_index(drop=True)
    valid_df = samples.iloc[n_train:n_train + n_valid].reset_index(drop=True)
    test_df = samples.iloc[n_train + n_valid:].reset_index(drop=True)

    return train_df, valid_df, test_df


def prepare_data(data_path, max_history_len=20):
    ...
    train_df, valid_df, test_df = split_by_time(samples)
...
class DINDataset(Dataset):
    def __init__(self, df, max_history_len=20):
...
def build_dataloaders(train_df, valid_df, test_df, max_history_len=20,
                      train_batch_size=1024, eval_batch_size=2048):
    train_loader = DataLoader(
        DINDataset(train_df, max_history_len=max_history_len),
    ...
    valid_loader = DataLoader(
        DINDataset(valid_df, max_history_len=max_history_len),
    ...
    test_loader = DataLoader(
        DINDataset(test_df, max_history_len=max_history_len),
        batch_size=eval_batch_size,
        shuffle=False,
        num_workers=0
    )
...
class DINAttention(nn.Module):
    ...
    def forward(self, hist_emb, target_emb, mask):
        ...
        batch_size, seq_len, embed_dim = hist_emb.shape
        ...
        interest = torch.sum(hist_emb * att_weights.unsqueeze(-1), dim=1)  # [B, D]
...
def train_model(model, train_loader, valid_loader, device, save_dir,
                num_epochs=8, lr=1e-3, weight_decay=1e-6):
    ...
    best_auc = -1.0
    best_path = os.path.join(save_dir, "din_movielens_best.pth")
...
@torch.no_grad()
def recommend_topk(model, user_id, history_movie_ids, candidate_movie_ids,
                   hour, device, max_history_len=20, topk=10):
    model.eval()
    ...
    for item_id in candidate_movie_ids:
        rows.append({
            "user_id": user_id,
    ...
    batch = {
        "user_id": torch.tensor([r["user_id"] for r in rows], dtype=torch.long).to(device),
        "target_movie_id": torch.tensor([r["target_movie_id"] for r in rows], dtype=torch.long).to(device),
        "hist_movie_ids": torch.tensor([r["hist_movie_ids"] for r in rows], dtype=torch.long).to(device),
        "hist_len": torch.tensor([r["hist_len"] for r in rows], dtype=torch.long).to(device),
        "hour": torch.tensor([r["hour"] for r in rows], dtype=torch.long).to(device)
    }
...
def run_recommendation(model, samples, info, device, save_dir):
    ...
    num_items = info["num_items"]
    candidate_movie_ids = list(range(1, min(300, num_items)))
    ...
    top10_path = os.path.join(save_dir, "top10_din_movielens.csv")
    full_rank_path = os.path.join(save_dir, "full_rank_din_movielens.csv")
...
def plot_training_curves(history, save_dir):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("DIN Training and Validation Loss")
    plt.legend()
    plt.grid(True)
    save_path = os.path.join(save_dir, "din_loss_curve.png")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("AUC")
    plt.title("DIN Validation AUC")
    plt.legend()
    plt.grid(True)
    save_path = os.path.join(save_dir, "din_valid_auc_curve.png")
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"[Save Figure] {save_path}")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("LogLoss")
    plt.title("DIN Validation LogLoss")
    plt.legend()
    plt.grid(True)
    save_path = os.path.join(save_dir, "din_valid_logloss_curve.png")
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"[Save Figure] {save_path}")
...
def plot_prediction_distribution(test_preds, test_labels, save_dir):
    ...
    plt.ylabel("Count")
    plt.title("DIN Prediction Distribution")
    plt.legend()
    plt.grid(True)

    save_path = os.path.join(save_dir, "din_prediction_distribution.png")
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"[Save Figure] {save_path}")
...
def plot_prediction_samples(test_preds, test_labels, save_dir, num_samples=200):
    ...
    plt.ylabel("Value")
    plt.title("DIN Predicted CTR vs True Label")
    plt.legend()
    plt.grid(True)

    save_path = os.path.join(save_dir, "din_prediction_samples.png")
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"[Save Figure] {save_path}")
...
def plot_topk_recommendation(top10, save_dir):
    ...
    plt.title("DIN Top-10 Recommendation Scores")
    plt.xticks(rotation=45)
    plt.grid(axis="y")

    save_path = os.path.join(save_dir, "din_top10_recommendation_scores.png")
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"[Save Figure] {save_path}")
...
@torch.no_grad()
def plot_attention_example(model, samples, device, save_dir, max_history_len=20):
    model.eval()
    ...
    plt.title(f"DIN Attention Weights, Target Movie ID = {int(row['target_movie_id'])}")
    plt.grid(axis="y")

    save_path = os.path.join(save_dir, "din_attention_example.png")
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
...
def save_all_figures(history, test_result, top10, model, samples, device, save_dir, max_history_len):
...
def save_experiment_results(best_auc, test_result, save_dir):
    result_path = os.path.join(save_dir, "din_movielens_result.txt")
...
def main():
    ...
    save_dir = make_dir(os.path.join(root, "outputs", "din"))

    max_history_len = 20

    print("=" * 80)
    print("[Project] MovieLens 1M DIN CTR Baseline")
    ...
    model = build_model(info=info, device=device)
