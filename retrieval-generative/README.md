...
在 HetRec LastFM 2K 的 92,824 条有效用户--艺术家交互上，每个种子为每个用户
分别留出验证和测试目标。原数据不含时间戳，因此不描述为时间切分。所有配置最多
训练 10 轮，只用验证 Recall@20 选 epoch，并在过滤历史后的完整物品库上测试一次。

| 配置 | Recall@20 | NDCG@20 | Coverage@20 |
| --- | ---: | ---: | ---: |
| BCE，0 个负样本 | 0.010621 +/- 0.005328 | 0.003335 | 0.170071 |
| BCE，1 个负样本 | 0.062489 +/- 0.003368 | 0.021801 | 0.052649 |
| BPR，1 个负样本 | **0.081076 +/- 0.003935** | **0.033771** | 0.002629 |
| BCE，5 个负样本，长尾加权 | 0.080191 +/- 0.000434 | 0.028890 | **0.004274** |

BPR-1 在三随机种子上取得最高总体 Recall/NDCG。长尾加权改善了 BCE 的稳定性和
覆盖率，但所有配置的稀有艺术家 Tail Recall@20 仍为 0；ID-only 模型也无法处理
约 12% 的 cold target，不能声称解决长尾或冷启动。完整 9 组结果见
...
- LastFM 主消融已完成三随机种子验证；其余教学规模实验仍需要更大训练规模复核。
